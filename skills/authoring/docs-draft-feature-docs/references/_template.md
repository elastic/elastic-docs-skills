---
area: <Area name, matching the H1 and the row in index.md>
verified: <YYYY-MM-DD, the day you last checked this file against the repos below>
verified_against:
  - docs-content
  - <elastic/kibana, elastic/elasticsearch, or whichever repos settle facts here>
status: references/status.md#<anchor>   # only when an in-flight transition affects this area; omit otherwise
---
...
> *Use a blockquote here only for a pointer a reader needs before the first heading: a `status.md` entry that affects every path below, or a sibling area file that has to be loaded alongside this one. The verification date belongs in the frontmatter, not here.*
...
*Give the identifier to search for alongside the path — a plugin name, a registration function, a step type. A path breaks silently on a refactor and then points at a directory that means something else; a symbol name usually survives the refactor, and when it does not, the search returns nothing loudly.*
...
An area file does its worst damage when it is confidently wrong, so prefer a form that fails loudly over one that rots quietly.

- **Expiry conditions, not dates.** Anything that stops being true on a known event goes in `status.md` with an *expires when* condition someone can check, such as an issue closing. A date is a guess about when the event will happen, and it goes wrong without anything looking wrong.
- **Checks, not snapshots.** "There is no `_snippets/` directory here" rots. "Check for a `_snippets/` directory before writing shared prose — at last check there was none" stays correct either way and still carries the useful prior. Write the instruction, then the observation.
- **Grep targets alongside paths.** Give the symbol as well as the path, so the path can be re-derived after a refactor instead of silently pointing somewhere wrong.
- **Substitutions, not literal product names.** When a name is scheduled to change, hardcoded prose is what breaks. Name the substitution and say never to type the literal string.
- **No counts.** "Five alerting systems" needs updating in every place it appears the moment a sixth exists. Name them instead.
- **No version pins for availability.** Which release carries a feature is `docs-applies-to-tagging`'s answer, from the plugin config. A version written here becomes a number people copy.
