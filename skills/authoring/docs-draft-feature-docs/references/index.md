...
It runs `context: fork`, so it returns a result rather than a running conversation. **Confirm a specialist is actually installed before routing to it** — a delegation to a skill that is not on the machine fails, and deriving from the area file or sibling pages is better than stopping.
