...
- **Agent Builder is a second seam.** `agent-builder/agents-and-workflows.md` and `agent-builder/tools/workflow-tools.md` describe workflows from the agent side and belong to `agent-builder.md`. Workflow authoring stays here; how an agent invokes a workflow does not.
- **This area spans two repos more often than most.** Five pages here already link out with `kibana://`, to `reference/advanced-settings.md` and `reference/connectors-kibana.md`. A settings change gating a workflows feature, or a new connector usable as an action step, lands in `elastic/kibana` rather than here — see `kibana-settings-reference.md` and Step 1e for the split and the merge order. Authoring content stays in docs-content.
